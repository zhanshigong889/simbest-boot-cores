...
public class AuthUserCacheServiceImpl implements IAuthUserCacheService {
    ...
    public static final String AUTH_USER_GLOBAL_KEY = "authuser:";

    public static final String AUTH_USER_PERMISSION_GLOBAL_KEY = "authuserpermission:";

    public static final String AUTH_USER_ACCESS_GLOBAL_KEY = "authuseraccess:";
    
    @Autowired
    private RedisTemplate<String, String> redisAuthUserKeyTemplate;

    //处理用户信息
    @Autowired
    private RedisTemplate<String, IUser> redisAuthUserTemplate;

    private ValueOperations<String, String> redisAuthUserKeyOps;

    private ValueOperations<String, IUser> redisAuthUserOps;
    ...
    //处理用户应用应用访问
    @Autowired
    private RedisTemplate<String, Boolean> redisUserAccessTemplate;

    private ValueOperations<String, Boolean> redisUserAccessOps;
    
    @PostConstruct
    public void init(){
        redisAuthUserKeyOps = redisAuthUserKeyTemplate.opsForValue();
        redisAuthUserOps = redisAuthUserTemplate.opsForValue();
        redisUserPermissionOps = redisUserPermissionTemplate.opsForValue();
        redisUserAccessOps = redisUserAccessTemplate.opsForValue();
    }
    ...
    public void saveOrUpdateCacheUser(IUser user) {
        ...
        //新增存放用户对象
        redisAuthUserOps.set(getUserCacheKey(user.getId()), user);
        log.debug("用户【{}】已通过键值【{}】完成缓存", user, getUserCacheKey(user.getId()));

        //存放username、preferredMobile、openid三组Key键
        if(StringUtils.isNotEmpty(user.getUsername())) {
            redisAuthUserKeyOps.set(getUserCacheKey(user.getUsername()), user.getId());
        ...
        if(StringUtils.isNotEmpty(user.getPreferredMobile())) {
            redisAuthUserKeyOps.set(getUserCacheKey(user.getPreferredMobile()), user.getId());
            log.debug("用户主键唯一标识【{}】已通过手机号码键值【{}】完成缓存", user.getId(), getUserCacheKey(user.getPreferredMobile()));
        }
        if(StringUtils.isNotEmpty(user.getOpenid())) {
            redisAuthUserKeyOps.set(getUserCacheKey(user.getOpenid()), user.getId());
    ...
    public void removeCacheUser(IUser user) {
        ...
        //清理存放用户对象
        redisAuthUserTemplate.expire(getUserCacheKey(user.getId()), 0, TimeUnit.NANOSECONDS);
        log.debug("已清理键值【{}】的缓存", getUserCacheKey(user.getId()));

        //清理username、preferredMobile、openid三组Key键
        if(StringUtils.isNotEmpty(user.getUsername())) {
            redisAuthUserKeyTemplate.expire(getUserCacheKey(user.getUsername()), 0, TimeUnit.NANOSECONDS);
            log.debug("已清理用户名键值【{}】的缓存", getUserCacheKey(user.getUsername()));
        }
        if(StringUtils.isNotEmpty(user.getPreferredMobile())) {
            redisAuthUserKeyTemplate.expire(getUserCacheKey(user.getPreferredMobile()), 0, TimeUnit.NANOSECONDS);
            log.debug("已清理手机号码键值【{}】的缓存", getUserCacheKey(user.getPreferredMobile()));
        }
        if(StringUtils.isNotEmpty(user.getOpenid())) {
            redisAuthUserKeyTemplate.expire(getUserCacheKey(user.getOpenid()), 0, TimeUnit.NANOSECONDS);
            log.debug("已清理openid键值【{}】的缓存", getUserCacheKey(user.getOpenid()));
        }
    }

    /**
     * 获取用户信息key键，以保证所有应用通用，所以不采用RedisUtil的方式区分应用标识
     * @param key
     * @return
     */
    ...
    /**
     * 获取用户权限key键，以保证所有应用通用，所以不采用RedisUtil的方式区分应用标识
     * @param username
     * @param appcode
     * @return
     */
    ...
    public void saveOrUpdateCacheUserAccess(String username, String appcode, Boolean isPermit) {
        ...
        redisUserAccessOps.set(getAuthUserAccessGlobalKey(username, appcode), isPermit);
    }
    ...
    public Boolean loadCacheUserAccess(String username, String appcode) {
        Boolean isPermit = redisUserAccessOps.get(getAuthUserAccessGlobalKey(username, appcode));
        if(null != isPermit) {
    ...
    public void removeCacheUserAccess(String username, String appcode) {
        ...
        Assert.notNull(appcode, "应用标识不允许为空!");
        //清理存放用户对象
        redisUserAccessTemplate.expire(getAuthUserAccessGlobalKey(username, appcode), 0, TimeUnit.NANOSECONDS);
        log.debug("已清理键值【{}】的缓存", getAuthUserAccessGlobalKey(username, appcode));
    }

    /**
     * 获取用户应用访问key键，以保证所有应用通用，所以不采用RedisUtil的方式区分应用标识
     * @param username
     * @param appcode
     * @return
     */
    ...
    public void removeCacheUserAllInformaitions(String username) {
        ...
        //2、清理用户权限信息
        Set<String> permissionKeys = redisUserPermissionTemplate.keys(AUTH_USER_PERMISSION_GLOBAL_KEY.concat(username).concat(ApplicationConstants.COLON).concat(ApplicationConstants.STAR));
        permissionKeys.forEach( k -> redisUserPermissionTemplate.expire(k, 0, TimeUnit.NANOSECONDS));
        //3、清理用户访问信息
        Set<String> accessKeys = redisUserPermissionTemplate.keys(AUTH_USER_ACCESS_GLOBAL_KEY.concat(username).concat(ApplicationConstants.COLON).concat(ApplicationConstants.STAR));
        accessKeys.forEach( k -> redisUserAccessTemplate.expire(k, 0, TimeUnit.NANOSECONDS));
    }
}
